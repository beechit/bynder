...
class BynderService implements SingletonInterface
{
    ...
    public function __construct(BynderClient $bynderClient)
    {
    ...
    public function getMediaInfo(string $uuid): array
    {
        ...
        if (($fileInfo = $this->getCache()->get('mediainfo_' . $uuid)) === false) {
            $fileInfo = $this->bynderClient->getAssetBankManager()->getMediaInfo($uuid)->wait();
    ...
    public function addAssetUsage(
        ...
        try {
            $usage = $this->bynderClient->getAssetBankManager()->createUsage([
    ...
    {
        try {
            $response = $this->bynderClient->getAssetBankManager()->deleteUSage([
                'integration_id' => $this->bynderIntegrationId,
                'asset_id' => $uuid,
                'uri' => $uri,
            ])->wait();
